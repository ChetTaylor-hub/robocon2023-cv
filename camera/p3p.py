...
class P3P(object):
    ...
    def angle(self, pixel_points, world_points):
        rvec, best_Ts = self.solve_P4P(pixel_points, world_points)
    ...
    def solve_P4P(self, pixel_points, world_points):
        ...
        undistorted_points = cv.undistortPoints(pixel_points, self.camera_matrix, self.distCoeffs)
        ...
        undistorted_points_extract = np.insert(undistorted_points_extract, 2, np.ones(len(undistorted_points_extract)),
                                               axis=1)  # 3*3
        ...
        for iteration in range(50):
            ...
            for i in range(3):
                ...
                for j in range(i + 1, 4):
                    ...
                    if (iteration > 3 and abs(D[i]) + eps_machine == abs(D[i]) and abs(D[j]) + eps_machine == abs(
                            D[j])):
                        A[loc] = 0
                    elif (abs(Aij) > tresh):
                        hh = D[j] - D[i]
    ...
    def length_solver(self, distances, cosines):
        ...
        for x in real_roots:
            ...
            x2 = x ** 2
            b1 = ((1 - a - b) * x2 + (q * a - q) * x + 1 - a + b) * (((r3 * (
                    a2 + ab * (2 - r2) - a_2 + b2 - 2 * b + 1)) * x + (r3q * (
                    2 * (b - a2) + a_4 + ab * (r2 - 2) - 2) + pr2 * (1 + a2 + 2 * (ab - a - b) + r2 * (
                    b - b2) + b2))) * x2 + (r3 * (
                    q2 * (1 - 2 * a + a2) + r2 * (b2 - ab) - a_4 + 2 * (a2 - b2) + 2) + r * p2 * (
                                                    b2 + 2 * (ab - b - a) + 1 + a2) + pr2 * q * (
                                                    a_4 + 2 * (b - ab - a2) - 2 - r2 * b)) * x + 2 * r3q * (
                                                                             a_2 - b - a2 + ab - 1) + pr2 * (
                                                                             q2 - a_4 + 2 * (
                                                                             a2 - b2) + r2 * b + q2 * (
                                                                                     a2 - a_2) + 2) + p2 * (
                                                                             p * (2 * (
                                                                             ab - a - b) + a2 + b2 + 1) + 2 * q * r * (
                                                                                     b + a_2 - a2 - ab - 1)))
